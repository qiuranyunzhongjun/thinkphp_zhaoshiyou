...
// 设置验证码
function show_verify($id=1,$config=''){
    if($config==''){
        $config=array(
            'codeSet'=>'1234567890',
            'fontSize'=>30,
 ...
 */
//function c($path='file',$format='empty',$maxSize='52428800'){
//    ini_set('max_execution_time', '0');
//    // 去除两边的/
//    $path=trim($path,'/');
//    // 添加Upload根目录
...
function pdf($html='<h1 style="color:red">hello word</h1>'){
    ...
    // 设置页眉显示的内容
    $pdf->SetHeaderData('logo.png', 60, 'demo.com', 'demo hi~', array(0,64,255), array(0,64,128));
    // 设置页眉字体
 ...
 * @return string 
 */
function think_ucenter_md5($str, $key = '|Qy1.,LHV=xqwlxYK&0eOAGM![5d}RbEp)q~n%*Z>+2BRc'){
